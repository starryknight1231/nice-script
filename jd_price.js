...
async function handleResponse(functionId,res){
    try{
        if(res.flag){
            ...
            console.log('本次成功保价' + succNum + '件, 共保价: ' + succAmount + '元');
            if(succNum > 0){
                $.message.insert('本次成功保价' +succNum + '件, 共保价: '+succAmount +'元');
            }
        }else if(res.responseMessage){
            console.log('保价申请失败:['+res.responseCode+'] '+ res.responseMessage);
            $.message.insert('保价申请失败:['+res.responseCode+'] '+ res.responseMessage);
        }else{
            console.log(`保价申请失败：${$.toStr(res)}`)
            $.message.insert(`保价申请失败：${$.toStr(res)}`)
        }
    }catch(err){
        console.log(`❌ 未能正确处理 ${functionId} 请求响应 ${err} `)
    }
}
